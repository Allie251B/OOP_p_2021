...
{
	if (sizeString + 1 >= capacityString)
	{
		resize();
		std::cout << sizeString << std::endl;
	}
	for (size_t i = sizeString + 1; i > index; i--)
	{
		ourString[i] = ourString[i - 1];
		std::cout << ourString[i] << std::endl;
	}
	ourString[index] = symbol;
	sizeString++;
...
{
	int mergedz_size = sizeString + secondString.sizeString;
	char* mergeStrings = new char[mergedz_size];
	mergeStrings = ourString;
	for (size_t i = 0; i < secondString.sizeString; i++)
	{
		mergeStrings[sizeString + i] = secondString.ourString[i];
	}
	mergeStrings[mergedz_size] = '\0';
	return mergeStrings;
}
